...
public class ProjectOnPlane : MonoBehaviour
{
    ...
    void Update()
    {
        if (timer > 2.0f)
        {
            ...
            vector = new Vector3(Random.Range(-5f, 5f), Random.Range(-5f, 5f), 0.0f);
            ...
            // Reset the timer.
            timer = 0.0f;
            Debug.Log(Vector3.Dot(vector, planeNormal));
            Debug.Log(Vector3.Dot(planeNormal, planeNormal));
        }
    ...
    void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawLine(Vector3.zero, planeNormal);
