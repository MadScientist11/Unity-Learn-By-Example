Shader "Unlit/Chart"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Assets/polyperfect/2D SDF Nodes/Sub Graphs/2D_SDF_CustomFunctions.hlsl"
            #include "Assets/UnityRayMarchingFramework/Shaders/ShaderLibs/Math.cginc"
            ...
            float ChartSDF(float2 uv, float2 polygon[6])
            {
                float color = 0.;
                float2 zero = 0.;
                Polygon_float(uv, 6, polygon[0], polygon[1], polygon[2], polygon[3], polygon[4], polygon[5], zero, zero,
            ...
            {
                float color = 0.;
                Segment_float(uv, float2(0, 0), dir * 2, color);
            ...
            float4 frag(Interpolators i) : SV_Target
            {
                ...
                float statsCoveredArea = ChartSDF(uvCentered, polygon);
