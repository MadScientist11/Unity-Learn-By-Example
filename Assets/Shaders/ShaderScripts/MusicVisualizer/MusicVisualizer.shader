Shader "Unlit/MusicVisualizer"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 cartesianToPolar(float2 cartesianCoord)
            {
                float r = length(cartesianCoord);
                float theta = atan2(cartesianCoord.y, cartesianCoord.x);
                return float2(r, theta);
            }

            float2 polarToCartesian(float2 polarCoord)
            {
                float x = polarCoord.x * cos(polarCoord.y);
                float y = polarCoord.x * sin(polarCoord.y);
                return float2(x, y);
            }

            float deform(float2 v0, float2 v1, float frequency, float amplitude)
            {
                float dot01 = dot(normalize(v0), normalize(v1));
                return amplitude * sin(dot01 * frequency);
            }

            float SawtoothWave_float(float In)
            {
